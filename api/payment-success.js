/* ANTRA — Pay4U success return (Vercel serverless function).
   Pay4U POSTs the result here. We verify the reply hash before
   showing success, so a fake success URL can't be trusted.
   Confirm the reverse-hash sequence with Pay4U's docs if needed. */
...
module.exports = function handler(req, res) {
  ...
  if (verified) {
    res.status(200).send(page("Thank you", "Your Antra order is confirmed. We’ll email you the details and begin hand-blending your ritual.", "✅"));
  } else {
    res.status(200).send(page("Payment could not be verified", "If money was deducted, it will be refunded automatically. Please contact us before re-ordering.", "⚠️"));
  }
};
